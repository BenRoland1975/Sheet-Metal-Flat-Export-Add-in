    ...
    public class PropertyStandardManager
    {
        ...
        public bool ApplyStandardsToActiveDocument(ModelDoc2 swModel, string currentStandardsVersion, bool silentMode = false)
        {
                            ...
                            else
                            {
                                ...
                                try
                                {
                                    ...
                                    if (wasOpen)
                                    {
                                        ...
                                        int errors = 0;
                                        int warnings = 0;
                                        swModel = swApp.OpenDoc6(filePath, docType, (int)swOpenDocOptions_e.swOpenDocOptions_Silent, "", ref errors, ref warnings);
                                        if (swModel == null)
                                        {
                                            LogMessage($"PDM: Failed to reopen document after checkout. Errors: {errors}, Warnings: {warnings}");
                                            if (!silentMode) MessageBox.Show("File was checked out but could not be reopened.", "Document Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                                            return false;
                                        }
                                    }
            ...
            try
            {
                ...
                swModel.Extension.CustomPropertyManager[""].Get6("Roeslein Standards Version", false, out tempVal, out tempResolvedVal, out tempWasResolved, out tempLinkToProperty);
            ...
            if (canWriteToFile)
            {
                try
                {
                    ...
                    standardsAppliedSuccessfully = ApplyAllPropertyStandards(swModel, currentStandardsVersion, silentMode);
        ...
        private bool ApplyAllPropertyStandards(ModelDoc2 swModel, string currentStandardsVersion, bool silentMode)
        {
            var allStandards = Settings.LoadSettings().PropertyStandards;
            ...
            ApplyFilteredStandards(swModel, applicableStandards, currentStandardsVersion);
        ...
        private void CheckAndMapMaterial(ModelDoc2 swModel)
        {
            try
            {
                if (swModel.GetType() != (int)swDocumentTypes_e.swDocPART)
                    return;
                // Add your sheet metal and material mapping logic here, or copy from backup
                // For now, just a stub to fix the error
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Applies the filtered property standards to the model, and sets the Roeslein Standards Version property.
        /// </summary>
        private void ApplyFilteredStandards(ModelDoc2 swModel, List<PropertyStandardSetting> standardsToApply, string currentStandardsVersion)
        {
            // Add your property application logic here, or copy from backup
            // For now, just a stub to fix the error
            // Set the Roeslein Standards Version property
            try
            {
                CustomPropertyManager swCustPropMgr = swModel.Extension.CustomPropertyManager[""];
                swCustPropMgr.Add3("Roeslein Standards Version", (int)swCustomInfoType_e.swCustomInfoText, currentStandardsVersion, (int)swCustomPropertyAddOption_e.swCustomPropertyDeleteAndAdd);
            }
            catch (Exception ex)
            {
                LogMessage($"Error in ApplyFilteredStandards: {ex.Message}");
            }
        }
    }
} 